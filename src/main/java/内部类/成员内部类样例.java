...
//        Node node = new Node(); 知识点：静态方法内实例化内部类 内部类必须为静态
        成员内部类样例 a = new 成员内部类样例();
        Node node = a.new Node();
        //知识点：如果想调用需用以上方式调用
        // 原因是：内部类可以自由访问外部类 而静态方法是不用实例化对象就可以调用的方法
//         你说如果实例化外部类 内部类还调用了外部类的变量 这就起了冲突
//         那么问题的根源就是一定要先实例化外部类 然后在这个实例化的外部类的基础上实例化内部类
//         这样逻辑关系才成立
    }
...
//        }
    ...
    /* 知识点总结：
            ...
            8.原因是：内部类可以自由访问外部类 而静态方法是不用实例化对象就可以调用的方法
               你说如果实例化外部类 内部类还调用了外部类的变量 这就起了冲突
               那么问题的根源就是一定要先实例化外部类 然后在这个实例化的外部类的基础上实例化内部类
               这样逻辑关系才成立
     */
